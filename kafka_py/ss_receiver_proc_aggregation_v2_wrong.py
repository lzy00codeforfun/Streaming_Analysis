...
# - create SparkContext and StreamingContext
    ...
    # words_vector = words.filter(words.vectorization != "")
    # vector_query = words_vector \
    #     .writeStream \
    #     .outputMode("append") \
    #     .format("kafka") \
    #     .option("checkpointLocation", "./vector_checkpoint") \
    #     .option("kafka.bootstrap.servers", bootstrap_servers) \
    #     .option("topic", "vector_insert") \
    #     .start()

    # wordCounts = words.groupBy("word").count()

    #bwy split hashtag & aggregation
    words_hash = words.filter(words.hashtag != "")

    words_hash = words_hash.withColumn(
        "hashtag",
        explode(
            split(words_hash.hashtag, " ")
        )
    )

    wordCounts = words_hash \
        .groupBy(window(words_hash.timestamp, "20 seconds", "5 seconds"),"hashtag") \
        .count()
